...
class EndNode(NetworkNode):
	...
	def transmit(self, env):
		...
		while True:
			...
			send_time = (self.slot[0] + self.counter_index) * (DataPacket(self.sf).rec_time + 2 * self.guard_time) + self.guard_time
...
def show_final_statistics():
	...
	output_file.write(f"Join Collisions: {nr_collisions}\n"
					  ...
					  + f"Number of nodes failed to connect to the network: {nodes_count - nr_joins if nodes_count - nr_joins >= 0 else 0}\n")
